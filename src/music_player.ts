...
import { initTradingFeature } from './trading_bars.js';
window.update_midi = update_midi;

declare global {
  interface Window {
    genConfig?: GenerationConfig;
    update_midi: typeof update_midi;
    converter: typeof converter;
    loadMidiTokens: typeof loadMidiTokens;
  }
}
...
async function main() {
  ...
  const trader = await initTradingFeature(chat, midi_loader);
  ...
  if (startButton) {
    startButton.addEventListener("click", async () => {
      ...
      while (!generationStopped) {
        ...
        const tokens = (await chat.chunkGenerate(configToUse))
          .split(",")
          .map((str) => parseInt(str));
